...
// ── Calcule les années min/max du catalogue ───────────────────────────────────
...
// ── Filtre par période d'archivage (watchedAt) ───────────────────────────────
...
export default function Films({
  ...
  // Sync bornes si les films changent
  useEffect(() => {
    setYearFrom(yearMin);
    setYearTo(yearMax);
  }, [yearMin, yearMax]);
  ...
  // Sync filtre initial (depuis dashboard)
  ...
  // ── Pipeline de filtrage (mémorisé pour perf) ─────────────────────────────
  const filtered = useMemo(() => {
    let result = validFilms;

    // 1. Filtre catégorie / flags
    ...
    // 2. Filtre période d'archivage
    result = filterByPeriod(result, period);

    // 3. Filtre années de sortie
  ...
  return (
    <div ref={topRef} className="flex-1 w-full relative min-h-screen transition-colors duration-700" style={ts.bgMain}>
      ...
      <div className="relative z-10 max-w-[1400px] mx-auto px-4 sm:px-6 py-8 sm:py-10">
        ...
        {/* ── BARRE DE RECHERCHE (discrète sur mobile) ── */}
        <div className="mb-4">
          {showSearch ? (
            <div className="flex items-center gap-3 rounded-xl border px-4 py-2.5 transition-all duration-300"
              ...
              <Search size={16} style={{ color: 'var(--accent-color)' }} className="shrink-0" />
              <input
                ref={searchRef}
                value={searchQuery}
        ...
        <div className="mb-3">
          <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap">
            <SlidersHorizontal size={13} className="shrink-0" style={ts.textMuted} />
            ...
            <button
              ...
              className={`flex items-center gap-1 px-2.5 py-1.5 text-[10px] sm:text-[11px] font-black uppercase tracking-wider border transition-all duration-200
                ${filter === 'favorite' ? 'border-yellow-400/60 text-yellow-300 bg-yellow-400/15' : ''}`}
            ...
            <button
              ...
              className={`flex items-center gap-1 px-2.5 py-1.5 text-[10px] sm:text-[11px] font-black uppercase tracking-wider border transition-all duration-200
                ${filter === 'heart' ? 'border-red-500/60 text-red-300 bg-red-500/15' : ''}`}
            ...
            {/* 6 sections */}
            {SECTION_FILTERS.map(({ key, label, count, color }) => {
        ...
        <div className="mb-8 sm:mb-10 flex flex-wrap gap-3 items-start">
          ...
          <div className="flex items-center gap-1.5 flex-wrap">
            <CalendarDays size={13} className="shrink-0" style={ts.textMuted} />
          ...
          {showYearFilter && (
            <div className="flex items-center gap-2 flex-wrap ml-0 sm:ml-2">
              <Calendar size={13} className="shrink-0" style={ts.textMuted} />
              <span className="text-[10px] font-black uppercase tracking-widest" style={ts.textMuted}>
                Sortie :
              </span>
              <div className="flex items-center gap-1.5">
                ...
                />
                <span className="text-[11px] font-black" style={ts.textMuted}>→</span>
                <input
                  type="number"
                  value={yearTo}
                  min={yearFrom}
        ...
        {filtered.length !== totalFilms && (
          <p className="text-[10px] font-black uppercase tracking-widest mb-4" style={ts.textMuted}>
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-32 text-center">
            ...
            <p className="font-black text-sm uppercase tracking-wider" style={ts.textMuted}>
          ...
          <div className="flex flex-col items-center justify-center py-24 sm:py-28 text-center">
            ...
            <p className="font-black text-base sm:text-lg uppercase tracking-wider mb-2" style={ts.textPrimary}>
              {totalFilms === 0 ? 'Aucun film archivé' : 'Aucun résultat'}
            </p>
            <p className="text-sm mb-6" style={ts.textSecondary}>
              {totalFilms === 0
